...
public class RomantoInteger13 {
	public int romanToInt(String s) {
		...
		int i = s.length() - 1;// right to left
		int rst = map.get(s.charAt(i--));// save last index
		while (i >= 0) {
			int digit1 = map.get(s.charAt(i));// left
			int digit2 = map.get(s.charAt(i + 1));// right
			if (digit1 >= digit2) {// II = 1 + 1
				rst += digit1;
			} else { // CM = 1000 - 100
				rst -= digit1;
			}
