 ...
 * chunk and symbol. A chunk is either an expression in parentheses, a variable,
 * or a non-negative integer. A variable is a string of lowercase letters (not
 * including digits.) Note that variables can be multiple letters, and note that
 * variables never have a leading coefficient or unary operator like "2x" or
 * "-x". Expressions are evaluated in the usual order: brackets first, then
 * multiplication, then addition and subtraction. For example, expression = "1 +
 * 2 * 3" has an answer of ["7"]. Input: expression = "e + 8 - a + 5", evalvars
 * = ["e"], evalints = [1] Output: ["-1*a","14"]
 * 
 * Input: expression = "e - 8 + temperature - pressure", evalvars = ["e",
 * "temperature"], evalints = [1, 12] Output: ["-1*pressure","5"]
 * 
 * Input: expression = "(e + 8) * (e - 8)", evalvars = [], evalints = [] Output:
 * ["1*e*e","-64"]
 * 
