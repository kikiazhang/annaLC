...
public class FrogJump403 {
	public boolean canCross(int[] stones) {
		...
		// can be taken from this stone. （stone位置，接下来能跳的长度set）
		HashMap<Integer, HashSet<Integer>> map = new HashMap<Integer, HashSet<Integer>>(stones.length);
		map.put(0, new HashSet<Integer>());
		map.get(0).add(1);// 开始一定是（0，1）
		...
		for (int i = 0; i < stones.length - 1; i++) {
			...
			for (int step : map.get(stone)) {
				int reach = step + stone;// 下一个能到的stone
				if (reach == stones[stones.length - 1]) {// 找到了
					return true;
				}
				HashSet<Integer> set = map.get(reach);// 写入reach的set可能
				if (set != null) {
					set.add(step);// cur step
					if (step - 1 > 0)// step - 1
						set.add(step - 1);
					set.add(step + 1);// step + 1
				}
			}
		}
