...
public class CourseScheduleII210 {
	...
	public int[] findOrder(int numCourses, int[][] prerequisites) {
		...
		Map<Integer, Integer> preMap = new HashMap<>();// course, pre count。保证preMap key有所有的course
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				int cur = q.poll();
				res[count++] = cur;
				Set<Integer> nexts = map.getOrDefault(cur, new HashSet<>());// 注意set default来遍历，不然报错
				for (int n : nexts) {
