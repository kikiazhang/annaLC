 ...
 * 
 * Example 1: Input: [[1,3], [0,2], [1,3], [0,2]] Output: true Explanation: The
 * graph looks like this: 0----1 | | | | 3----2 We can divide the vertices into
 * two groups: {0, 2} and {1, 3}. Example 2: Input: [[1,2,3], [0,2], [0,1,3],
 * [0,2]] Output: false Explanation: The graph looks like this: 0----1 | \ | | \
 * | 3----2 We cannot find a way to divide the set of nodes into two independent
 * subsets.
 */
public class IsGraphBipartite785 {
	public boolean isBipartite(int[][] graph) {
		// neighbor不在一个color
		int n = graph.length;// 有几个
		// graph表示index连接哪几个index
		int[] color = new int[n];
		Arrays.fill(color, -1);

		for (int start = 0; start < n; ++start) {
			if (color[start] == -1) {
				Stack<Integer> stack = new Stack();// 先进后出
				stack.push(start);
				color[start] = 0;
				...
				while (!stack.empty()) {
					...
					for (int nei : graph[node]) {
						if (color[nei] == -1) {// 没看过，就push，color取反
							stack.push(nei);
