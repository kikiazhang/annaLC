...
public class OpenLock752 {
	public int openLock(String[] deadends, String target) {
		...
		queue.offer("0000");
		queue.offer(null);// trigger the layer
		...
		while (!queue.isEmpty()) {
			...
			if (node == null) {
				depth++;
				if (queue.peek() != null) {// there is more nodes in other layer
					queue.offer(null);
				}
			} else if (node.equals(target)) {
				return depth;
			} else if (!dead.contains(node)) {
				for (int i = 0; i < 4; i++) {// 4 digit
					for (int d = -1; d <= 1; d += 2) {// -1 1
						int y = ((node.charAt(i) - '0') + d + 10) % 10;// also make 0 -> 9
						String nei = node.substring(0, i) + ("" + y) + node.substring(i + 1);
						if (!seens.contains(nei)) {// not seen, add to queue
							seens.add(nei);
							queue.offer(nei);
						}
					}
